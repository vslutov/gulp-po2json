...
const createFile = function (contents) {
  const base = path.normalize(path.resolve(__dirname, '../fixtures'))
  return new Vinyl({
...
describe('gulp-po2json', function () {
  it('should match po2json output', function (done) {
    po2json.parseFile(path.join(__dirname, 'fixtures/es.po'), {stringify: true}, function (err, data) {
      if (err) {
        done(err)
        return
      }

      fs.readFile(path.join(__dirname, 'fixtures/es.po'), function (err, esPo) {
        if (err) {
          done(err)
          return
        }
